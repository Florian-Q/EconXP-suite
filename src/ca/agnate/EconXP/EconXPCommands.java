...
public class EconXPCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    	                                                     // If there's nothing after the /econxp, give the info for XP
                                                    	    // for even all the available command: /econxp ? or /econxp help (reserved to the admin)
        if (args.length == 0) {
        	
        	OfflinePlayer target = validatePlayer( sender , sender.getName() );
        	
    		sender.sendMessage( ChatColor.GREEN + "[" + ChatColor.DARK_GREEN + "EconXP" + ChatColor.GREEN + "] " +        //the green of EconXP
    		ChatColor.GREEN + "you have : "+ ChatColor.DARK_GREEN +
    		plugin.getExpToLevel100(plugin.getExp(target)) + " " + plugin.money +                                         //rise and name of money
    		ChatColor.GREEN + " worth : " + ChatColor.DARK_GREEN + plugin.getExp(target) + ChatColor.GREEN + " exp");
